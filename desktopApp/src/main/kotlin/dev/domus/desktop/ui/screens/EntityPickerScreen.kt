...
fun EntityPickerScreen(
    ...
    LaunchedEffect(session) {
        if (entities.isEmpty()) session.repository.refresh()
    }
    ...
    Scaffold(
        ...
        Box(
            ...
            if (entities.isEmpty()) {
                ...
                val filtered = entities.values
                    .filter { e ->
                        query.isBlank() ||
                            e.friendlyName.contains(query, ignoreCase = true) ||
                            e.entityId.contains(query, ignoreCase = true)
                    }
                        ...
                        items(filtered, key = { it.entityId }) { entity ->
                            ...
                            Row(
                                modifier = Modifier
                                    .fillMaxWidth()
                                    .clickable {
                                        selection = if (isSelected) selection - entity.entityId
                                        else selection + entity.entityId
                                    }
                                    .padding(
                                ...
                                Checkbox(
                                    ...
                                    onCheckedChange = { checked ->
                                        selection = if (checked) selection + entity.entityId
                                        else selection - entity.entityId
                                    },
                                )
                                Column(modifier = Modifier.padding(start = DesignTokens.Spacing.sm.dp)) {
                                    Text(entity.friendlyName, style = MaterialTheme.typography.bodyLarge)
                                    Text(entity.entityId, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                                }
                            }
                        }
                    }
